...
def hop_skip_jump_attack(
    ...
    def hsja(sample, target_label, target_image, prev_indice):
        if target_label is None:
            if model_name == 'ds2':
                out, outsizes = model_fn(sample, input_length)
                _, original_label = torch.max(out, 2)
            else:
                _, original_label = torch.max(model_fn(sample), 2)
        ...
        def decision_function(images):
            ...
            for i in range(0, len(images), batch_size):
                ...
                if model_name == 'ds2':
                    prob_i, _ = model_fn(batch, input_length)
                    # prob_i = model_fn(batch, input_length)
                else:
                    prob_i = model_fn(batch)
                prob.append(prob_i)
            prob = torch.cat(prob, dim=0)
            if target_label is None:
                return torch.max(prob, dim=2)[1] != original_label
            else:
                return torch.max(prob, dim=2)[1] == target_label
        ...
        for j in np.arange(num_iterations):
            ...
            #         )
            #     )
            noise = perturbed - sample
            noise = noise.reshape(-1)
            g_eval = np.zeros(shape=noise.size())
            if use_TD == 1:
                h = 3
                if total_q <= 3:  # initalize
                    var_list = np.array(range(0, sample.reshape(-1).size()[0]), dtype=np.int32)
                    indices = var_list[
                        np.random.choice(var_list.size, int(batch_size), replace=False)]  # selected coord index
                else:  # after the first iteration
                    _, _, indice = td(ori_input, sample, sample.size()[-1], h, batch_size, prev_indice)
                    prev_indice += indice.tolist()
                    indices = np.array(indice)
            else:
                indices = np.random.choice(sample.size()[-1] * sample.size()[-2], size=sample_per_draw, replace=False)
            for ind in indices:
                g_eval[ind] = noise[ind]
            g_hat = g_eval.reshape(sample.size())

        return g_hat, prev_indice
    ...
    for i, x_ in enumerate(x):
        ...
        if y_target is not None:
            ...
            pert, prev_indice = hsja(x_, y_target[i], image_target[i], prev_indice)
        else:
            if image_target is not None:
                pert, prev_indice = hsja(x_, None, image_target[i], prev_indice)
            else:
                # untargeted attack without an initialized image.
                pert, prev_indice = hsja(x_, None, None, prev_indice)
        adv_x.append(pert)
    # return torch.cat(adv_x, 0)
    return adv_x[0], prev_indice
...
def approximate_gradient(
    ...
    fval = 2.0 * decisions[:, 0].view((decisions.shape[0],) + (1,) * len(shape)) - 1.0
...
def binary_search_batch(
    ...
    while torch.max((highs - lows) / thresholds) > 1:
        ...
        lows = torch.where(decisions == 0, mids, lows)[0][0:1]
        highs = torch.where(decisions == 1, mids, highs)[0][0:1]
